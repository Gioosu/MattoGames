  ...
  return (
    <div
      ...
      style={{
        ...
        // Spazio extra in fondo: la BottomNav è fissa e coprirebbe l'ultima
        // riga di card senza questo margine.
        ...
        {games.map((game, index) => {
          ...
          if (isGhost) {
            return (
              ...
              >
                <span className="font-display font-bold text-xl text-cream">{game.name}</span>
                <p className="font-body text-sm text-muted mt-1">{game.description}</p>
              </div>
            );
          }
          ...
          return (
            <div
              ...
              className={[
                ...
                isPlayable
                  ? [
                      "bg-surface border border-white/5 shadow-xl shadow-black/30 cursor-pointer",
                      cardTilt(index),
            ...
            >
              <div className="flex items-start justify-between gap-3">
                ...
                <button
                  type="button"
                  aria-label={`Informazioni su ${game.name}`}
                  onClick={(e) => {
                    e.stopPropagation();
              ...
              <span
                className={[
                  ...
                  isPlayable ? "bg-glow/15 text-glow" : "bg-white/5 text-muted",
                ].join(" ")}
              >
                {game.players}
              </span>
      ...
      {selectedGame && (
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-5">
          <div className="bg-surface rounded-3xl p-6 max-w-sm w-full shadow-xl">
            ...
            <p className="font-body text-sm text-muted mt-4">{selectedGame.longDescription}</p>
